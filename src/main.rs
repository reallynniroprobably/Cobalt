...
fn main() {
    ...
    fs::write(format!("tests/{test}_output.txt"), output.iter().map(|s| s.raw.clone() + " " + (if s.token_type == lexer::TokenType::Identifer { "identifer" } else { "other" })).collect::<Vec<String>>().join("\n"))
